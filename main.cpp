...
static void showMainMenuBar(){
    ...
    if (ImGui::BeginMainMenuBar()){
        ...
        if (ImGui::BeginMenu("Output Display")){
            // pressing this button should open a new menu to select the display to output to
            int monitor_num = 0;
            GLFWmonitor ** monitors = glfwGetMonitors(&monitor_num);
            int i = 0;
            if (monitor_num < 2){
                ImGui::Text("Less than 2 monitors connected, cannot project to a second display");
            }else {
                while(i < monitor_num){
...
int main(int, char**)
{
    ...
    char lyricbuf[lyricbuflen+1] = {};

    const int titlebuflen =64;
    char titlebuf[titlebuflen+1] = {};
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // when the user searches, the results will have to be populated into a special string type that holds the results.
        // then the result will pull the song name out of a hash table or somthing, and edit that song. 
        static int song_num = 1;
        static bool ret = false;

        const char *items[] = {"song", "song1", "song2"};
        ImGui::ListBox("Song List",&song_num,items, IM_ARRAYSIZE(items), IM_ARRAYSIZE(items) );
        ...
        */
        ...
        for (int n = 0;n <sched_song_list.size(); n++)
        {
            ...
            if (ImGui::IsKeyReleased(261)&& ImGui::IsItemHovered()){
                ...
                // if a user presses delete when they are hovering over a song, delete it.
        ...
        static bool ret = false;
            ImGui::ListBox("Song List",&song_num, all_song_list.data(), all_song_list.size()  );
        ...
        if (ImGui::Button("Save (does not work)")){
            show_nothing_msg = !show_nothing_msg;
        }
        if (show_nothing_msg){

            ImGui::Text("That Did Nothing");
        }
        ImGui::End();
