...
int init_and_open_db(char *fname, sqlite3 **out_db) {
  ...
  const char *picture_sql_cmd = "create table if not exists main.pictures(id "
                                "integer primary key asc,name text,data blob);";
  ...
  const char *sched_song_link_sql_cmd =
      "create table if not exists mainsched_song_links("
      "song_id INTEGER,"
      "sched_id INTEGER,"
